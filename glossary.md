...
| **bean (full definition)** | An object whose whole lifecycle — creation, dependency-wiring, holding, destruction — is managed by Spring inside the `ApplicationContext`, rather than by you writing `new`. You declare it a bean and declare what it needs; the container builds and injects. | Lesson 04 |
| **coupling** | How much one piece of code depends on the exact details of another. *Tight* = depends on concrete specifics, can't change one without disturbing the other; *loose* = depends only on a stable promise, so either can change freely. | Lesson 04 |
| **tight coupling** | When a class knows and constructs the concrete classes it depends on (e.g. `new EmailReceiptSender()` inside a service), welding high-level logic to low-level detail and making it rigid to change and untestable in isolation. | Lesson 04 |
| **Dependency Injection (DI)** | The technique that carries out IoC: an object's dependencies are handed *to* it (usually via its constructor) instead of created inside it. Plain Java — Spring only automates it across a whole app. | Lesson 04 |
| **Inversion of Control (IoC)** | The principle that a component does not choose and fetch its own collaborators; that control is handed to something outside it. Same idea as a framework calling your code — the "Hollywood Principle." DI is the usual way to achieve it. | Lesson 04 |
| **Hollywood Principle** | "Don't call us, we'll call you." The framework/container is in control and calls into your objects handing them what they need, rather than your objects calling out to fetch dependencies — the plain-English name for IoC. | Lesson 04 |
| **interface (as a dependency)** | A named list of abilities with no bodies — a pure promise of *what* can be done, not *how*. Depending on the interface (the "what") instead of a concrete class (the "how") is "programming to an interface" and is what makes implementations swappable. | Lesson 04 |
| **programming to an interface** | Pointing your dependencies at a promise (an interface) rather than a specific implementation, so any class keeping the promise can be substituted without editing the depender. | Lesson 04 |
| **container (Spring)** | The engine that, at startup, finds every bean, works out what each needs, creates them in dependency order, and injects each one's dependencies — the `ApplicationContext` is this container. It is *where* dependency injection happens. | Lesson 04 |
| **test double / stub** | A stand-in object used in a test in place of a real collaborator, so the test stays fast, offline, and predictable — e.g. a fake `ReceiptSender` that just records the call. Only injectable because of loose coupling. | Lesson 04 |
